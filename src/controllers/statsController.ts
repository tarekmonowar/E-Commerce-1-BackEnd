...
export const getDashboardStats = TryCatch(async (req, res, next) => {
  ...
  if (myNodeCache.has(key)) {
    stats = JSON.parse(myNodeCache.get(key) as string);
  } else {
    //*--------------------------------------------------------------------- code start here ----------------------------------------
    ...
    myNodeCache.set(key, JSON.stringify(stats));
  } //end block
...
export const getPieCharts = TryCatch(async (req, res, next) => {
  ...
  if (myNodeCache.has(key)) {
    charts = JSON.parse(myNodeCache.get(key) as string);
  } else {
    //
    //
    //
...
export const getBarCharts = TryCatch(async (req, res, next) => {
  ...
  if (myNodeCache.has(key)) {
    ...
    const lastSixMonthProductsPromise: any = Product.find({
      createdAt: {
        $gte: sixMonthAgo,
        $lte: today,
      },
...
export const getLineCharts = TryCatch(async (req, res, next) => {
  ...
  if (myNodeCache.has(key)) {
    ...
    const lasttwelveMonthProductsPromise: any = Product.find({
      createdAt: {
        $gte: twelveMonthAgo,
        $lte: today,
      },
    ...
    const lasttwelveMonthOrdersPromise: any = Order.find({
      createdAt: {
        $gte: twelveMonthAgo,
        $lte: today,
      },
